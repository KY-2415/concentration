...
if($turn==0){
  ...
  ?>
  <div style="text-align:center;">
  <form action="com_2.php" method="get">
  ...
  for($i=0;$i<52;$i++){
    //最初にめくったカードと同じ数字をもつカードが一度めくられたカード（$card[$i][1]==4）の中にあったらそれを選択する
    if($card[$i][1]==4&&$card[$position][0]%13==$card[$i][0]%13){
      $random = $i;
      break;
    }
  }
  if(isset($random)){ //上の条件で位置が決まらなかったらランダムで決定する。
  }else{
    $shuffle = range(0,51);
    shuffle($shuffle);
    for($i=0;$i<52;$i++){
